// Datei: test_canvas.cpp

#include "gip_mini_catch.h"

#include "MyCanvas.h"

void do_test(MyCanvas &canvas, size_t size_x, size_t size_y)
{
    size_t x = 0, y = 0;
    char c = '0';
    canvas.set(x, y, c);
...
void do_check(const MyCanvas &canvas, size_t size_x, size_t size_y)
{
    size_t x = 0, y = 0;
    char c = '0';
    REQUIRE(canvas.get(x, y) == c);
...
{
    const size_t size_x = 5, size_y = 3;

    MyCanvas canvas{size_x, size_y, new char[size_x * size_y - 1]};
...
{
    const size_t size_x = 7, size_y = 4;
...
{
    const size_t size_x = 9, size_y = 6;
...
{
    const size_t size_x1 = 9, size_y1 = 6;

    MyCanvas canvas1{size_x1, size_y1};
    do_test(canvas1, size_x1, size_y1);

    const size_t size_x2 = size_x1 + 9, size_y2 = size_y1 + 6;
